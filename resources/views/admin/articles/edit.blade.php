    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.articles.update", [$article->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $article->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <label for="pdf">{{ trans('cruds.article.fields.pdf') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.article.fields.pdf_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="publish_at">{{ trans('cruds.article.fields.publish_at') }}</label>
                <input class="form-control date {{ $errors->has('publish_at') ? 'is-invalid' : '' }}" type="text" name="publish_at" id="publish_at" value="{{ old('publish_at', $article->publish_at) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id" required>
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $article->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
...
@if(isset($article) && $article->pdf)
      var file = {!! json_encode($article->pdf) !!}
          this.options.addedfile.call(this, file)
      file.previewElement.classList.add('dz-complete')
